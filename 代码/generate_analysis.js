...
const OUTPUT_FILE = path.join(__dirname, '../.md/specs/project-analysis/详细项目源码全面剖析.md');
...
const modules = [
    {
        name: "2.1 用户模块",
        desc: "包含用户登录、注册、个人信息修改、密码维护等核心功能相关的控制器、服务层和前端视图。",
    ...
    {
        name: "2.3 借阅模块",
        desc: "整个系统的核心流程，包括图书借阅申请、审核、归还、报损等流转操作。",
    ...
    {
        name: "2.6 系统核心功能模块",
        desc: "通用配置、异常处理、拦截器、安全工具、微信推送通知、定时任务以及整体的通用布局视图等代码。",
...
for (const file of allFiles) {
    ...
    if (!assigned) {
        moduleFilesMap["2.6 系统核心功能模块"].push(file);
        usedFiles.add(file);
    }
}
...
"---\n\n" +
"## 目录\n" +
"- [第一部分：功能模块分层级部署展开](#第一部分功能模块分层级部署展开)\n" +
...
for (const mod of modules) {
    ...
    for (const file of files) {
        ...
        markdownContent += "#### 所在文件相对路径：`" + relPath + "`\n\n";
...
markdownContent += "> **至此，从前端点击行为-响应-业务落库-超时管控 的全部链路与项目源码绑定呈现完毕。由于涉及完整代码生成，文件篇幅巨大。请通过全局搜索检索具体类目。**\n";

if(!fs.existsSync(path.dirname(OUTPUT_FILE))) {
    fs.mkdirSync(path.dirname(OUTPUT_FILE), { recursive: true });
}
fs.writeFileSync(OUTPUT_FILE, markdownContent);
console.log('Document generation successful! Total modules processed: ' + modules.length);
