...
import com.booksharing.book.mapper.BorrowMapper;
...
import com.booksharing.book.service.BorrowService;
...
public class BorrowServiceImpl implements BorrowService {
    ...
    public R addBorrow(Borrow borrow) {
        ...
        if (currentUser != null && currentUser.getCreditScore() != null && currentUser.getCreditScore() < 60) {
            return R.error("您的信用分低于60分，已被限制借阅！如有特殊情况请线下联系管理员。");
        }
        ...
        // 【新增关键修复】：检查是否已有待审核或待交接的借阅记录！防止多个用户同时申请同一本书
        Map<String, Object> checkParams = new HashMap<>();
        checkParams.put("bookId", borrow.getBookid());
        checkParams.put("stateList", "0,4"); // 0-待审核, 4-待交接
        int pendingCount = borrowMapper.selectCount(checkParams);
        if (pendingCount > 0) {
            return R.error("这本书目前有其他用户正在申请或交接中，请稍后再试");
        }
    ...
    public R auditBorrow(Integer borrowId, Integer state, String feedback) {
        ...
        if (state == 1) { // 审核通过
            // 【新增关键修复】：双重检查！审核通过时再检查是否已有其他待交接的申请！防止并发问题
            Map<String, Object> checkParams = new HashMap<>();
            checkParams.put("bookId", borrow.getBookid());
            checkParams.put("state", 4); // 4-待交接
            int pendingCount = borrowMapper.selectCount(checkParams);
            if (pendingCount > 0) {
                return R.error("审核失败！这本书目前已有其他用户正在交接中！");
            }

            if (book.getInventory() <= 0) return R.error("审核失败：库存不足");

            book.setInventory(book.getInventory() - 1);
            if (book.getInventory() == 0) book.setIsborrowed(1); // 标记为无库存
            ...
            // 【核心修复】：同意的瞬间，把 applytime 刷新为现在！3天交接倒计时重新开始算
            borrow.setApplytime(new Date());
    ...
    public R forcePenalize(Integer borrowId) {
        ...
        if (badUser != null && badUser.getOpenId() != null) {
            WechatPushUtils.pushMessage(
                    badUser.getOpenId(),
                    "⚠️ 信用降级与强制收回通知",
                    "警告！因您逾期未归还《" + book.getBookname() + "》，系统已强制结束借阅，并扣除您 10 信用分！<br/>当前剩余信用分：" + badUser.getCreditScore() + "分。<br/>⚠️ 若低于60分将永久封禁借阅权限！如有特殊情况，请线下联系管理员说明并申请恢复信用分。"
            );
        }
