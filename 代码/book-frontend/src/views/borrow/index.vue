  ...
  <div class="app-container">
      ...
      <el-card class="table-card" shadow="hover">
        <el-table :data="list" v-loading="listLoading" style="width: 100%" :header-cell-style="{background:'#f5f7fa', color:'#606266'}">
          ...
          <el-table-column label="接收人" width="120" align="center">
            <template slot-scope="{row}">
              <el-tag size="small" effect="light" style="font-weight: bold;">{{ row.username }}</el-tag>
            </template>
          </el-table-column>

          <el-table-column label="信用评级" width="110" align="center">
            <template slot-scope="{row}">
              <el-tag size="small" effect="dark" :type="getCreditType(row.creditScore)" style="border-radius: 10px;">
                <i class="el-icon-s-data"></i> {{ row.creditScore != null ? row.creditScore : 100 }} 分
              </el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="学号" width="140" align="center">
            <template slot-scope="{row}">
              <span style="color: #606266; font-family: monospace; font-size: 14px;">
                {{ row.studentid || row.studentId || '暂无数据' }}
              </span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="160" align="center" fixed="right">
            <template slot-scope="{row}">
              ...
              <el-button v-else size="mini" type="text" disabled>
                {{ row.state === 5 ? '已撤销' : (row.state === 6 ? '已报损' : (row.state === 7 ? '已失效' : '已结束')) }}
              </el-button>
            </template>
          </el-table-column>
        </el-table>
                  ...
                  <div class="card-content">
                    <div class="book-details">
                      ...
                      <div class="status-line">
                        <span v-if="item.state === 0" class="status-tag">审核中...</span>
                        <span v-else-if="item.state === 4" class="status-tag pending" style="color: #E6A23C;">审核已通过，请联系发布者拿书</span>
                        <span v-else-if="item.state === 1" class="status-tag active">正在漂流中</span>
                      ...
                      <div v-if="item.state === 4" class="secret-code-box">
                        <span>向发布者出示此暗号提书:</span>
                        <strong class="code-text">{{ item.secretCode }}</strong>
                      </div>

                      <div v-if="item.state === 4" style="margin-top: 10px;">
                        <el-button size="mini" type="primary" plain icon="el-icon-chat-line-square" @click="handleNudge(item)">无法前往？协商新地点</el-button>
                      </div>
                    </div>
              ...
              <el-timeline-item v-for="(item, index) in list" :key="item.borrowid" :timestamp="item.applytime" placement="top" :color="item.state === 0 ? '#909399' : (item.state === 4 ? '#F56C6C' : '#67C23A')">
                <el-card class="footprint-card" :class="{'is-pending': item.state === 4 || item.state === 0}" shadow="hover" style="border-left-color: #F56C6C">
                  <div class="card-content">
                    <div class="book-details">
                      <h3 class="book-title">
                        📘 《{{ item.bookname }}》 被 <span style="color:#409EFF">{{ item.username }}</span> 申请了

                        <el-tooltip :content="item.creditScore < 60 ? '信用极低，建议拒绝' : '信用良好'" placement="top">
                          <el-tag size="mini" effect="dark" :type="getCreditType(item.creditScore)" style="margin-left: 8px; border-radius: 10px; vertical-align: middle;">
                            信用评级: {{ item.creditScore != null ? item.creditScore : 100 }}
                          </el-tag>
                        </el-tooltip>
                      </h3>
                      ...
                      <div class="status-line" style="margin-top: 8px;">
                        <span v-if="item.state === 0" class="status-tag" style="color: #F56C6C; font-weight: bold;"><i class="el-icon-bell"></i> 正在等待您亲自审批</span>
                        <span v-else-if="item.state === 4" class="status-tag pending" style="color: #E6A23C;"><i class="el-icon-time"></i> 已同意，等待对方出示暗号</span>
                        <span v-else-if="item.state === 1" class="status-tag" style="color: #67C23A;"><i class="el-icon-reading"></i> 对方正在阅读中</span>
                      </div>
                    </div>
                    ...
                    <div class="book-action">
                      ...
                      <el-button v-else-if="item.state === 4" size="medium" type="danger" round @click="openHandoverDialog(item)" style="box-shadow: 0 4px 10px rgba(245, 108, 108, 0.3);">
                        <i class="el-icon-scan"></i> 核销提书暗号
                      </el-button>
    ...
    <el-dialog :visible.sync="dialogVisible" width="420px" custom-class="glass-dialog" :show-close="false">
      <div class="dialog-header-custom">
        <div class="icon-ring"><i class="el-icon-lock"></i></div>
        <h3 style="margin: 15px 0 5px 0; color: #303133;">面对面交接验证</h3>
        <p style="color: #909399; font-size: 13px; margin: 0;">请输入借阅者出示的 6 位数字暗号，确认交接</p>
      </div>
      ...
      <div slot="footer" style="text-align: center; padding-bottom: 10px;">
        <el-button round @click="dialogVisible = false" style="margin-right: 15px;">取消</el-button>
    ...
    <el-dialog title="📚 读完并传递 (成为新一任持书人)" :visible.sync="returnDialogVisible" width="550px" custom-class="glass-dialog">
      <el-form :model="returnForm" label-position="top">
        <el-form-item label="您的漂流感悟 (所有人可见的漂流足迹)">
          <el-input type="textarea" :rows="3" v-model="returnForm.returnMsg" placeholder="这本书给你带来了什么启发？留下你的足迹吧..." />
        </el-form-item>
        <el-form-item label="您的交接说明 (非常重要！绝对保密)">
          <el-input type="textarea" :rows="2" v-model="returnForm.contactInfo" placeholder="例如：我是男生，住在5栋302，或者加我微信 xxxx" />
          <div style="font-size: 12px; color: #909399; margin-top: 5px;">
            <i class="el-icon-info"></i> 这本书现在由您保管！请留下您的联系方式，等待下一位有缘人申请。只有审核通过后，对方才能看到此信息。
          </div>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="returnDialogVisible = false" round>暂不传递</el-button>
        <el-button type="primary" @click="submitReturn" round>确认传递，等待下一位</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      returnForm: { borrowId: null, bookId: null, returnMsg: '', contactInfo: '' }
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      if (!this.checkPermission(['admin'])) {
        if(this.activeTab === 'borrow') params.userId = this.id;
        else params.uploaderId = this.id;
      }
    ...
    handleAudit(row, targetState) {
      let actionName = targetState === 1 ? '同意借出' : '驳回申请';
      this.$confirm(`确认${actionName}该请求吗？`, '提示', { type: targetState === 1 ? 'success' : 'warning' }).then(() => {
        request({ url: '/borrow/audit', method: 'post', params: { borrowId: row.borrowid, state: targetState } }).then(res => {
          if (res.code === 0 || res === 1) {
            let successMsg = targetState === 1 ? '审批通过！已成功通知对方。' : '已成功驳回该申请。';
    ...
    submitHandover() {
      ...
      request({ url: '/borrow/verifyCode', method: 'post', params: { borrowId: this.currentRowId, secretCode: this.inputSecretCode }
      }).then(res => {
        this.submitLoading = false;
        if (res.code === 0 || res === 1) { this.$message.success("🎉 暗号核销正确！交接成功！"); this.dialogVisible = false; this.getList(); }
        else { this.$message.error(res.msg || "❌ 暗号错误！"); }
      }).catch(() => { this.submitLoading = false; });
    },

    handleReturn(row) {
      this.returnForm = { borrowId: row.borrowid, bookId: row.bookid, returnMsg: '', contactInfo: '' };
      this.returnDialogVisible = true;
    },

    submitReturn() {
      if(!this.returnForm.returnMsg) return this.$message.warning("写点感悟吧，让漂流足迹更丰富！");
      if(!this.returnForm.contactInfo) return this.$message.warning("请务必留下您的交接说明！");

      request({ url: '/borrow/returnBook', method: 'post', params: {
          borrowId: this.returnForm.borrowId, bookId: this.returnForm.bookId,
          returnMsg: this.returnForm.returnMsg, contactInfo: this.returnForm.contactInfo
        }
      }).then(res => {
        if(res.code === 0 || res === 1) {
          this.$message.success("传递成功！您已正式成为这本书的新主人！");
          this.returnDialogVisible = false; this.getList();
        } else { this.$message.error(res.msg || "操作失败，请重试"); }
      });
    },

    handleCancel(row) {
      this.$prompt('请输入撤销/放弃的原因：', '撤销交接', {
        confirmButtonText: '确认撤销', cancelButtonText: '暂不撤销', type: 'warning'
      }).then(({ value }) => {
        request({ url: '/borrow/cancel', method: 'post', params: { borrowId: row.borrowid, reason: value || '无法完成线下交接' }
        }).then(res => {
          if (res.code === 0 || res === 1) { this.$message.success('已成功撤销，书籍已释放！'); this.getList(); }
        });
      });
    },

    handleNudge(row) {
      this.$prompt('如果无法进入(如异性宿舍)，请在此留下您的建议碰面地点：', '请求协商交接', {
        confirmButtonText: '发送给对方', cancelButtonText: '取消', customClass: 'glass-message-box'
      }).then(({ value }) => {
        if (!value) return this.$message.warning("留言不能为空哦");
        request({ url: '/borrow/nudge', method: 'post', params: { borrowId: row.borrowid, message: value }
        }).then(res => {
          if (res.code === 0 || res === 1) { this.$message.success('发送成功！请耐心等待对方联系你。'); }
        });
      });
    },

    handleReportLoss(row) {
      this.$prompt('书籍遗失/损坏是大家都不想看到的，请如实填写原因（将通知发布者并彻底终止此书漂流）：', '登记报损', {
        confirmButtonText: '确认报损', cancelButtonText: '取消', type: 'error',
        inputPlaceholder: '例如：不小心掉进水里泡坏了，非常抱歉'
      }).then(({ value }) => {
        ...
        request({ url: '/borrow/reportLoss', method: 'post', params: { borrowId: row.borrowid, reason: value } }).then(res => {
          if(res.code === 0 || res === 1) { this.$message.success("报损登记成功，该书已退出漂流体系。"); this.getList(); }
        });
      });
    },

    handleUrge(row) {
      this.$confirm('确定要催促该读者尽快阅读并传递吗？（将向对方发送提醒）', '一键催还', { type: 'warning' }).then(() => {
        request({ url: '/borrow/urge', method: 'post', params: { borrowId: row.borrowid } }).then(res => {
          if(res.code === 0 || res === 1) { this.$message.success("催促通知已成功发送！"); }
        });
      });
    }
  }
...
.admin-view {
  .filter-container { border-radius: 16px; border: none; margin-bottom: 20px; .filter-header { display: flex; justify-content: space-between; align-items: center; .filter-title { font-size: 18px; font-weight: bold; color: #303133; } .filter-actions { display: flex; gap: 10px; } } }
...
.user-view {
  ...
  .footprint-card {
    border-radius: 16px; border: none; transition: transform 0.3s, box-shadow 0.3s; background: #fff; display: flex; flex-direction: column;
    &.is-active { border-left: 5px solid #409eff; background: #fdfdff; }
    &.is-pending { border-left: 5px solid #E6A23C; background: #fff8eb; }
    ...
    .book-action { margin-left: 20px; display: flex; flex-direction: column; justify-content: center; gap: 6px; }
  }
}
